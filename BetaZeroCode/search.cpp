...
static void ClearForSearch(S_BOARD *pos, S_SEARCHINFO *info) {
	...
	for(index = 0; index < 13; ++index) for(index2 = 0; index2 < BRD_SQ_NUM; ++index2) pos->searchHistory[index][index2] = 0;
	for(index = 0; index < 2; ++index) for(index2 = 0; index2 < MAXDEPTH; ++index2) pos->searchKillers[index][index2] = 0;
...
static int AlphaBeta(int alpha, int beta, int depth, S_BOARD *pos, S_SEARCHINFO *info, int DoNull) {
	...
	for(MoveNum = 0; MoveNum < list->count; ++MoveNum) {
		...
		if(FoundPv==TRUE){
			...
			if(Score > alpha && Score < beta) Score= -AlphaBeta(-beta, -alpha , depth-1, pos , info , TRUE);
		}
		else Score = -AlphaBeta( -beta, -alpha, depth-1, pos, info, TRUE);
		TakeMove(pos);
		...
		if(Score > BestScore) {
			...
			if(Score > alpha) {
				...
				if(!(list->moves[MoveNum].move & MFLAGCAP)) pos->searchHistory[pos->pieces[FROMSQ(BestMove)]][TOSQ(BestMove)] += depth;
			}
		}
    }
